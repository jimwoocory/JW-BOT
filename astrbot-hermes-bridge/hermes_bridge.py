...
@register("hermes_bridge", "hermes_bridge", "AstrBot 到 Hermes Agent 的 Webhook 桥接")
class HermesBridgePlugin(Star):
    ...
    async def _handle_hermes_response(self, request):
        ...
        try:
            ...
            return aiohttp.web.Response(
                status=200, text=json.dumps({"status": "ok", "user_id": user_id})
            )
    ...
    async def _send_to_qq(self, user_id: str, message: str):
        ...
        try:
            ...
            logger.debug(
                f"[HermesBridge] 准备发送消息到 QQ {user_id}: {message[:50]}..."
            )

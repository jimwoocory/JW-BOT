    ...
    def get_or_create_session(self, platform_user: PlatformUser) -> str:
        ...
        try:
            ...
            if row:
                session_id = row[0]
                logger.debug(
                    f"Found existing session {session_id} for {platform_user_id}"
                )
            else:
                # 2. 创建新会话
...
async def create_router_api_server(
    router: SessionRouter, host: str = "0.0.0.0", port: int = 8788
):
    ...
    async def create_session(request):
        ...
        try:
            ...
            return web.json_response(
                {"session_id": session_id, "platform_user": platform_user.to_dict()}
            )
        except Exception as e:

...
class DreaminaPlugin(Star):
    ...
    async def _execute_dreamina(self, command: list, timeout: int = 300, _retry: int = 3) -> tuple[bool, str]:
        """执行即梦 CLI 命令
        ...
        for attempt in range(1, _retry + 1):
            try:
                ...
                if "ExceedConcurrencyLimit" in output and attempt < _retry:
                    wait = 10 * attempt
                    logger.warning(f"触发并发限制，{wait} 秒后重试（{attempt}/{_retry}）")
        ...
        if success:
            ...
            if url_match:
                ...
                try:
                    ...
                    yield event.plain_result("图片已保存，可用 /图片转视频 <描述> 生成动画")
                except Exception as e:
    ...
    async def text2video(self, event: AstrMessageEvent, prompt: str = ""):
        """文生视频功能"""
        if not prompt:
            yield event.plain_result("请提供视频描述，例如：/生成视频 海浪拍打礁石，慢动作")
            return
    ...
    async def image2video(self, event: AstrMessageEvent, prompt: str = ""):
        ...
        if not self.last_image_path or not os.path.exists(self.last_image_path):
            yield event.plain_result("没有找到最近生成的图片，请先用 /生成图片 生成一张图片")
            return

        command = [
            "image2video",
            "--image", self.last_image_path,
            "--prompt", prompt or "animate the scene",
            "--duration", "5",
            "--poll", "900",
        ]

        yield event.plain_result(f"正在将图片动画化：{prompt or '（默认动效）'}\n这可能需要较长时间...")
    ...
    async def query_task(self, event: AstrMessageEvent, submit_id: str):
        """查询特定任务结果"""
        if not submit_id:
            yield event.plain_result("请提供任务 ID，例如：/查询即梦任务 3f6eb41f425d23a3")
            return
        ...
        if success:
            ...
            if url_match:
                ...
                try:
                    ...
                    self.last_image_path = tmp.name
                    yield event.image_result(image_url)
                    yield event.plain_result("图片已保存，可以直接说「把这张图做成视频」")
                except Exception as e:
                    logger.warning(f"图片下载失败：{e}")
                    yield event.image_result(image_url)
            else:
